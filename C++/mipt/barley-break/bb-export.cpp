...
using namespace std;
int N;
const unsigned long long END_STATUS16 = 0xfedcba987654321;
const unsigned long long END_STATUS9 =  0x000000087654321;
#define END_STATUS(N) ((N == 3)? END_STATUS9: END_STATUS16)
...
if (!fin || !fout) {std::cerr << "input/output error"; return 1;}
...
while (1) {
    if (open_stat.begin () -> second.game_stat == END_STATUS(N)) break; //bb-find_path completed
...
bool solvabiliti (unsigned long long _stat)
{
unsigned inversions = 0, count = 0, number = 0, null_pos = 0;
unsigned long long _stat2 = 0;

if (N == 3) /*shift zero to right-down corner*/
    {
    ++inversions; //inversions will increase by 3 because null will be in rd corner
    for (;;)
        {
        null_pos = where_null (_stat);
        if (null_pos >= 6) break;
        number = what_number (_stat, null_pos + N);
        _stat = _stat + number*mypow (null_pos) - number*mypow (null_pos +N);
        }

    for (;;)
        {
        null_pos = where_null (_stat);
        if (null_pos %3 == 2) break;

        number = what_number (_stat, null_pos + 1);
        _stat = _stat + number*mypow (null_pos) - number*mypow (null_pos + 1);
        }
    }

for (int i = 0; i < N*N; ++i, _stat /= 16)
    if (_stat - _stat /16*16 == 0) inversions += i/N + 1;
        ...
        _stat2 /= 16;
        for (int j = i + 1; j < N*N; ++j, _stat2 /= 16)
